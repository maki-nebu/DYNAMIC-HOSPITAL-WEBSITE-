...
class UserHomeController extends Controller
{
    public function index()
    {
        try {
            ...
            return view('welcome', compact('testimonies', 'featured_news', 'banners', 'setting', 'events', 'galleries', 'blogs', 'departments', 'head_message', 'publications', 'directorates', 'services', 'visible'));
        } catch (\Throwable $th) {
            $setting = Setting::find(1);
...
public function initiatives()
        ...
        try {
            ...
            if ($request->hasFile('attachments')) {
                if (isset($attachments)) {
                    ...
                    $application->arrachments = $attachmentsname;
                } else {
   ...
   public function addComment(Request $request, $slug)
    ...
    public function privacypolicy()
    {
        try {
            $setting = Setting::find(1);
            return view('front.privacypolicy', compact('setting', 'services'));
        } catch (\Throwable $th) {
            return redirect()->back()->with('infoMsg', $th->getMessage());
        }
    }
    ...
    public function about()
    {
        try {
            ...
            $departments = Department::where('status', 1)->with(['directorates' => function ($query) {$query->where('status', 1);}])->get();


            return view('front.about', compact('setting', 'partners', 'testimonies', 'histories', 'accreditations','departments', 'head'));
        } catch (\Throwable $th) {
            return redirect()->back()->with('infoMsg', $th->getMessage());
        }
    }
    ...
    public function privacy()
    {
        try {
            $setting = Setting::find(1);
            return view('front.privcay', compact('setting'));
        } catch (\Throwable $th) {
            toast($th, 'error');
            return redirect()->back()->with('infoMsg', $th->getMessage());
        }
